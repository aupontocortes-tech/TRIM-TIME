/**
 * Apaga cobranças locais e reinicia assinatura (trial) para testar contratação do zero.
 * Uso: node scripts/reset-barbershop-billing.mjs teste
 */
...
if (!query) {
  console.error("Uso: node scripts/reset-barbershop-billing.mjs <nome-ou-slug>")
  process.exit(1)
}
...
console.log("\nFaça login na barbearia e contrate de novo (cartão fake no sandbox).")
